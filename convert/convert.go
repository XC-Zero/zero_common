...
func GetAnyStructTag(obj any, tag string) []map[string]string {
	if obj == nil {
		return nil
	}
	var ref reflect.Type
	var res = make([]map[string]string, 0, 0)
	refTy := reflect.TypeOf(obj)
	switch refTy.Kind() {
	...
	case reflect.Ptr, reflect.Slice, reflect.Array:
		...
		GetAnyStructTag(reflect.New(ref).Elem().Interface(), tag)
	case reflect.Struct:
		break
	default:
		return nil
